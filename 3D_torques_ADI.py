...
def plot_3d_torque_comparison(output_path="profiles", short_mode=False):
    """
    Creates a plot comparing 3D and 3D thin simulation torques.
    ...
    """
    print("Starting 3D torque comparison...")
    if short_mode:
    ...
    # Define simulations based on the provided list (with corrected names)
    simulation_3d_thin = [
        "cos_bet1d6_ss15_fi05_nodust_r0615_PaardekooperFig2_nu1dm11_pramp10_3Dthin",
        #"cos_bet1d6_ss0_fi0_r0615_z05_PaardekooperFig2_nu1dm11_COR_LR_3D_CHECKZ",
        "cos_bet1d6_ss0_fi0_nodust_r0615_PaardekooperFig2_nu1dm11_pramp10_3Dthin", 
        "cos_bet1d6_ss15_fim05_nodust_r0615_PaardekooperFig2_nu1dm11_pramp10_3Dthin"
    ]
    
    simulation_3d = [
        "cos_bet1d6_ss0_fi0_r0615_z05_PaardekooperFig2_nu1dm11_COR_LR_3D_CHECK",
        #"cos_bet1d6_ss0_fi0_r0615_z05_PaardekooperFig2_nu1dm11_COR_LR_3D_CHECKZ",
        "cos_bet1d6_ss15_fi05_r0615_z05_PaardekooperFig2_nu1dm11_COR_LR_3D",
        "cos_bet1d6_ss15_fim05_r0615_z05_PaardekooperFig2_nu1dm11_COR_LR_3D"
    ]
    
    print(f"Processing {len(simulation_3d_thin)} 3D thin simulations and {len(simulation_3d)} 3D simulations")
    ...
    if short_mode:
        ...
        rolling_window_size = 100  # Normal mode - 5 orbits smoothing
    ...
    if short_mode:
        ...
        plot_x_max = 1000  # Normal mode: Show full 100 orbits as in original script
    ...
    for i, sim in enumerate(simulation_3d_thin):
        ...
        # For early time: Use Equation45 (non-linear, unsaturated)
        print("Computing theoretical torques for early time (unsaturated)...")
        predicted_torque_early, _, GAM0 = patched_compute_theoretical_torques(parameters, qp, eq_label="Equation45")
        ...
        if not short_mode:
            ...
            print(f"Theoretical torques: early (unsaturated)={predicted_torque_early}, late (saturated)={predicted_torque_late}")
        else:
            print(f"Theoretical torque (unsaturated)={predicted_torque_early}")
        ...
        # Use thicker line for 3D thin simulations
        line_width = 4.0 if '3Dthin' in sim else 2.0
        ...
        ax.plot([0, theo_line_length], 
                ...
                color=param_colors[color_key], linestyle=':', linewidth=line_width)
    ...
    # Place the text labels at an appropriate position
    text_y_position = min(all_data_min) - 0.5
    
    # Add appropriate theoretical line labels based on mode
    if short_mode:
        ax.text(theo_line_length/2, text_y_position, r"$\Gamma_{\mathrm{unsaturated}}$", fontsize=12)
    else:
        ax.text(theo_line_length/2, text_y_position, r"$\Gamma_{\mathrm{unsaturated}}$", fontsize=12)
        ax.text(plot_x_max - theo_line_length/2, text_y_position, r"$\Gamma_{\mathrm{saturated}}$", fontsize=12, horizontalalignment='center')
    ...
    if short_mode:
        plot_title = f'Total torque on planet in 3D simulations - Early evolution (Smoothing: {smoothing_time_orbits:.1f} orbits)'
    else:
        plot_title = f'Total torque on planet in 3D simulations (Smoothing: {smoothing_time_orbits:.1f} orbits)'
    
    plt.title(plot_title, fontsize=14)
    
    # Add text explaining the simulation parameters
    print("Adding simulation parameters text...")
    plt.figtext(0.5, 0.01, r'$q = 1.26 \times 10^{-5}, h = 0.05, \gamma = 5/3, b/h = 0.4$', 
                fontsize=12, ha='center')
    
    # Create appropriate filename based on mode
    if short_mode:
        output_filename = f"{output_path}/torque_3D_adiabatic_comparison_early.pdf"
    else:
        output_filename = f"{output_path}/torque_3D_adiabatic_comparison.pdf"
...
def main():
    parser = argparse.ArgumentParser(description="Generate adiabatic torque comparison plots")
    ...
    plot_3d_torque_comparison(output_path=args.output, short_mode=args.short)
