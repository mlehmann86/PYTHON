...
def plot_results(simulations_3D, simulations_2D, labels):
    ...
    ylims = [
        [1e-5, 1e-2],  # First panel
        [1e-5, 1e-2],  # Second panel
    ]
    ...
    for i in range(len(simulations_3D)):
        ...
        for metallicity, data in data_3D.items():
            ...
            alpha_r = apply_smoothing(alpha_r, time, smoothing_window, start_time)
            ...
            line, = axes[i].plot(time, alpha_r, label=f"$Z={metallicity}$", color=colors[metallicity], linestyle='-')
        ...
        for metallicity, data in data_2D.items():
            ...
            axes[i].plot(time, alpha_r, color=colors[metallicity], linestyle='--')
            
           
        axes[i].set_ylim(ylims[i])
        axes[i].set_yscale('log')
        axes[i].set_ylabel(r"$\alpha_{r}$")
    ...
    for ax in axes:
        ...
        ax.set_ylabel(r"$\alpha_{r}$", fontsize=20)
    ...
    # Save the plot
    output_filename = "alpha_r_comparison.pdf"
...
if __name__ == "__main__":
    ...
    plot_results(
        [simulations_tau_0_01_3D, simulations_tau_0_1_3D],
        [simulations_tau_0_01_2D, simulations_tau_0_1_2D],
        labels
    )
