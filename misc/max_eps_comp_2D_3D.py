...
def plot_results(simulations_3D, simulations_2D, labels):
    ...
    ylims = [
        [1e-2, 1e2],  # First panel
        [1e-3, 1e3],  # Second panel
        [1e-2, 1e4]   # Third panel
    ]
    ...
    for i in range(len(simulations_3D)):
        ...
        for metallicity, data in data_3D.items():
            ...
            line, = axes[i].plot(time, max_epsilon, label=f"$Z={metallicity}$", color=colors[metallicity], linestyle='-')
            
            # Highlight Roche density exceedance
            if roche_times is not None:
                for roche_time in roche_times:
                    ...
                    if len(epsilon_value_at_time) > 0:
                        axes[i].scatter(roche_time, epsilon_value_at_time, color=line.get_color(), marker='o', s=100)
        ...
        for metallicity, data in data_2D.items():
            ...
            axes[i].plot(time, max_epsilon, color=colors[metallicity], linestyle='--')
            
            # Highlight Roche density exceedance (for 2D as well)
            if roche_times is not None:
                for roche_time in roche_times:
                    epsilon_value_at_time = max_epsilon[time == roche_time]
                    if len(epsilon_value_at_time) > 0:
                        axes[i].scatter(roche_time, epsilon_value_at_time, color=colors[metallicity], marker='o', s=100)

        axes[i].set_ylim(ylims[i])
        axes[i].set_yscale('log')
        axes[i].set_ylabel(r"$\epsilon_{\max}$")
    ...
    for ax in axes:
        ...
        ax.set_ylabel(r"$\epsilon_{\max}$", fontsize=20)
    ...
    # Save the plot
    output_filename = "max_epsilon_comparison.pdf"
