...
# In torque_profile.py
import numpy as np

# FILE: torque_profile.py
import numpy as np

# FILE: torque_profile.py
import numpy as np

def compute_torque_gravity(gasdens, xgrid, ygrid, zgrid, r_p, phi_p, qp, h0, thick_smooth, flaring_index, use_hill_cut=False):
    """
    Computes the radial torque density profile.
    Includes radially-dependent smoothing and an optional Hill Cut.
    """
    ny, nx, nz = gasdens.shape
    ...
    for j in range(ny):
        ...
        for i in range(nx):
            ...
            # Radially-dependent smoothing length
            ...
            for k in range(nz):
                ...
                # --- START: OPTIONAL HILL CUT IMPLEMENTATION ---
                hill_cut_factor = 1.0 # Default to full force
                if use_hill_cut:
                    dist_to_planet = np.sqrt(dist_to_planet_sq)
                    # Hill Radius (assuming MSTAR=1.0)
                    hill_radius = r_cell * (qp / 3.0)**(1./3.)
                    
                    if hill_radius > 1e-9: # Avoid division by zero if qp=0
                        ratio = dist_to_planet / hill_radius
                        if ratio < 0.5:
                            hill_cut_factor = 0.0
                        elif ratio < 1.0:
                            # Smoothly ramp down the force inside the Hill sphere
                            hill_cut_factor = np.sin((ratio - 0.5) * np.pi)**2
                # --- END: OPTIONAL HILL CUT IMPLEMENTATION ---
                
                denom = (dist_to_planet_sq + smoothing_sq)**1.5
                fphi = -qp * dy / (denom + 1e-30)
                
                # Apply the Hill Cut factor to the force
                fphi *= hill_cut_factor
                
                rho = gasdens[j, i, k]
                torque_density[i] += rho * r_cell**2 * fphi * dphi * dz

    return torque_density

# The rest of your torque_profile.py file can remain as is.
...
def main():
    ...
    if args.comparison:
        ...
        for sim in simlist:
            ...
            # Plot
            line, = ax.plot(xgrid, grav, label=label)
            all_grav_profiles.append(grav)
            if all_xgrid is None:
                all_xgrid = xgrid

            # Mark boundaries in a list for later plotting.
            domain_boundaries.append((rmin_val, rmax_val, line.get_color()))

            # Compute total torque as well and plot horizontal line
            total_grav = np.sum(grav) * (xgrid[1] - xgrid[0])
            print(f"  → Total gravitational torque (from snapshots): {total_grav:.6e}")
            ax.hlines(total_grav, xgrid[0], xgrid[-1], colors=line.get_color(),
            ...
            try:
                ...
                if np.any(mask):
                    ...
                    ax.hlines(total_internal, xgrid[0], xgrid[-1], colors=line.get_color(),
