...
def plot_results(simulation_dirs_with_labels):
    ...
    for sim_dir, label, color in zip(simulation_dirs, labels, colors):
        ...
        if sim_dir in data:
            ...
            # Handle the standard solid line first for full range (Standard Simulation and No Dust Feedback)
            if label == "Standard Simulation":
                axes[0].plot(time, alpha_r, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                axes[1].plot(time, max_epsilon, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                #axes[2].plot(time, (max_vz - min_vz) / aspectratio, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                axes[2].plot(time, rms_vz / aspectratio, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

            elif label == "No Dust Feedback":
                axes[0].plot(time, alpha_r, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                axes[1].plot(time, max_epsilon, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                #axes[2].plot(time, (max_vz - min_vz) / aspectratio, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
                axes[2].plot(time, rms_vz / aspectratio, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

            # Now handle the split cases with dashed lines for "Feedback on after 400 orbits" and "Feedback off after 728 orbits"
            elif label == "Feedback On After 400 Orbits":
                time_before_400 = time[time <= 400]
                time_after_400 = time[time >= 400]

                alpha_before_400 = alpha_r[time <= 400]
                alpha_after_400 = alpha_r[time >= 400]

                epsilon_before_400 = max_epsilon[time <= 400]
                epsilon_after_400 = max_epsilon[time >= 400]

                #vz_before_400 = (max_vz[time <= 400] - min_vz[time <= 400]) / aspectratio
                #vz_after_400 = (max_vz[time >= 400] - min_vz[time >= 400]) / aspectratio
                vz_before_400 = (rms_vz[time <= 400]) / aspectratio
                vz_after_400 = (rms_vz[time >= 400]) / aspectratio

                # Plot solid part AFTER 400 orbits and dashed before 400 orbits
                axes[0].plot(time_before_400, alpha_before_400, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[0].plot(time_after_400, alpha_after_400, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

                axes[1].plot(time_before_400, epsilon_before_400, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[1].plot(time_after_400, epsilon_after_400, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

                axes[2].plot(time_before_400, vz_before_400, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[2].plot(time_after_400, vz_after_400, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

            elif label == "Feedback Off After 728 Orbits":
                time_before_728 = time[time <= 728]
                time_after_728 = time[time >= 728]

                alpha_before_728 = alpha_r[time <= 728]
                alpha_after_728 = alpha_r[time >= 728]

                epsilon_before_728 = max_epsilon[time <= 728]
                epsilon_after_728 = max_epsilon[time >= 728]

                #vz_before_728 = (max_vz[time <= 728] - min_vz[time <= 728]) / aspectratio
                #vz_after_728 = (max_vz[time >= 728] - min_vz[time >= 728]) / aspectratio
                vz_before_728 = (rms_vz[time <= 728] ) / aspectratio
                vz_after_728 = (rms_vz[time >= 728] ) / aspectratio

                # Plot solid part AFTER 728 orbits and dashed before 728 orbits
                axes[0].plot(time_before_728, alpha_before_728, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[0].plot(time_after_728, alpha_after_728, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

                axes[1].plot(time_before_728, epsilon_before_728, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[1].plot(time_after_728, epsilon_after_728, linestyle='solid', color=color, linewidth=default_linewidth, label=label)

                axes[2].plot(time_before_728, vz_before_728, linestyle='dashed', color=color, linewidth=default_linewidth)
                axes[2].plot(time_after_728, vz_after_728, linestyle='solid', color=color, linewidth=default_linewidth, label=label)
    ...
    for ax in axes:
        ...
        ax.axvline(x=370, color='black', linestyle='dashed', linewidth=1)

    # Set the vertical axes to logarithmic scale and adjust the ranges
    axes[0].set_yscale('linear')
    axes[0].set_ylim(1e-3, 6e-3)
    ...
    # Save the plot
    output_filename = f"simulation_column_plot.pdf"
...
if __name__ == "__main__":
    simulation_dirs_with_labels = [
        ...
        "cos_b1d0_us_St5dm2_Z1dm2_r6H_z08H_fim053_ss203_3D_2PI_stnew_LR150_nfb",
        "cos_b1d0_us_St5dm2_Z1dm2_r6H_z08H_fim053_ss203_3D_2PI_stnew_LR150_nfb_fbon",
        "cos_b1d0_us_St5dm2_Z1dm2_r6H_z08H_fim053_ss203_3D_2PI_stnew_LR150_fboff",
        "Standard Simulation",
        "No Dust Feedback",
        "Feedback On After 400 Orbits",
        "Feedback Off After 728 Orbits"
    ]
